import { CreditCard, Delete, Warning } from "@mui/icons-material";
import {
  Button,
  Divider,
  Grid,
  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text" onClick={()=>{handleNewAddress({isAddingNewAddress:false})}}
        >
          Cancel
        </Button>
      </Stack>
...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    const remainingBalance=localStorage.getItem("balance")-getTotalCartValue(items);
  ...
  useEffect(()=>{
    if (!token){
      ...
      const addressesLoader = async () => {
        ...
        if (addressList) {
          setAddresses({ ...addresses, all: addressList });
        }
      };
      addressesLoader();
    }
        ...
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              (
                addresses.all.map((address) => (
                  <Box key={address._id} className=
                  {
                    addresses.selected===address._id ? "address-item selected" : "address-item not-selected"
                  } 
                  display="flex"  justifyContent="space-between" mt={3} mb={1} p={2} 
                  onClick={()=>{
                  ...
                  >
                    ...
                    <Button startIcon={<DeleteIcon />} onClick={()=>{deleteAddress(token, address._id)}}>
                      Delete
                    </Button>
                  </Box>
                )) // end of map function
            ...
            <Button
              ...
              variant="contained"
              onClick={()=>{
                  const requestValid = validateRequest(items,addresses)
                  let checkoutSuccess=false;
                  if(requestValid){
                      checkoutSuccess=performCheckout(token,items,addresses);
                      if(checkoutSuccess){
                        enqueueSnackbar("Order placed successfully",{variant:"success"});
                        history.push("/thanks");
                      }
                      else{
                        console.log("perform checkout not successfukl..");
                      }
                  }
                  else{
